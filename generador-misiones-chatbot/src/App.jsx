...
const App = () => {
  ...
  const handleChatSend = useCallback(async (e) => {
    ...
    if (!hasWebGPU || !engineRef.current) {
      setStatusText('⚠️ El modelo aún no está listo para el chat.');
      return;
    }
    ...
    try {
      const prompt = buildChatPrompt(userMsg.content, newHistory);
      const messages = [
  { role: 'system', content: 'Eres un asistente experto en sostenibilidad. No uses corchetes ni plantillas. Responde solo con el contenido final en español latino. Sé concreto y no divagues. No mezcles tecnologías distintas a la consultada (p.ej., si es eólica, no hables de solar salvo comparación breve). Prioriza el contexto proporcionado y evita inventar datos. Cierra tu respuesta con un punto final.' },
      ...
      const stream = await engine.chat.completions.create({
        ...
        temperature: 0.1,
        max_tokens: 120,
        stop: ['\nUsuario:', '\nAsistente:', 'Pregunta del usuario:']
      });
      ...
      while (true) {
        ...
        if (delta) {
          ...
          const partial = sanitizePartialChat(accum);
          // Actualizamos el último mensaje del asistente con el parcial
      ...
      const finalText = finalizeChatAnswer(accum, userMsg.content);
      setChatMessages(prev => {
        const next = [...prev];
        next[assistantIdx] = { role: 'assistant', content: finalText || 'No logré generar una respuesta.' };
        return next;
      });
  ...
  }, [buildChatPrompt, chatInput, chatMessages, hasWebGPU, isChatLoading]);
